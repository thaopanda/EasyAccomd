...
class HostRegistrationView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if(serializer.is_valid()):
            ...
            message = 'Có người cho thuê mới cần phê duyệt'
...
class HostProfileView(APIView):
    ...
    class HostProfileSerializer(serializers.ModelSerializer):
        class Meta:
            model = Host
            fields = ['username', 'email', 'date_joined', 'last_login', 'fullname', 'identication', 'address', 'phoneNumber', 'has_update_permission']
...
class HostUpdateProfileView(APIView):
    ...
    def put(self, request, format=None):
        ...
        if(host.has_update_permission is False):
            return Response(f'not ok')
...
class ConfirmedHostAccount(APIView):
    ...
    def put(self, request, pk, format=None):
        ...
        if(serializer.is_valid()):
            serializer.save(is_confirmed=True, has_update_permission=False)
            message = 'Tài khoản của bạn đã được phê duyệt'
            Notification.objects.CreateNotification(host, message )
            return Response(f'ok')
        return Response(f'not ok')
