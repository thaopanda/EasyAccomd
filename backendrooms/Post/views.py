...
detailAddress_validator = (
    '^[a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂẾưăạảấầẩẫậắằẳẵặẹẻẽềềểếỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ0-9,\s]+$')
describeAddress_validator = (
    '^[a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂẾưăạảấầẩẫậắằẳẵặẹẻẽềềểếỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ0-9,\s]+$')
other_validator = (
    '^[a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂẾưăạảấầẩẫậắằẳẵặẹẻẽềềểếỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ0-9,\s]+$')
...
class CreatePostView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if(serializer.is_valid()):
            ...
            serializer.save(hostName=user, is_confirmed=False, numberOfRented=0, expiredDate=datetime.now(
            )+timedelta(days=serializer.validated_data['expiredDate']))
            adminUser = MyUser.objects.get(email='EasyAccomd@gmail.com')
            message = 'Có bài đăng mới cần phê duyệt'
...
class UpdateRentStatusView(APIView):
    ...
    def put(self, request, pk, format=None):
        ...
        if(post.is_confirmed is True):
            serializer = self.UpdateRentStatusSerializer(
                post, data=request.data)
            if(serializer.is_valid()):
                availableRoom = post.numberOfRoom - \
                    serializer.validated_data['numberOfRented']
                if(availableRoom >= 0):
...
class ExtendExpiredDateView(APIView):
    ...
    def put(self, request, pk, format=None):
        ...
        if(post.expiredDate.replace(tzinfo=None) < datetime.now().replace(tzinfo=None)):
            serializer = self.ExtendExpiredDateSerializer(
                post, data=request.data)
            if(serializer.is_valid()):
                serializer.save(is_confirmed=False, expiredDate=datetime.now(
                )+timedelta(days=serializer.validated_data['expiredDate']))
                adminUser = MyUser.objects.get(email='EasyAccomd@gmail.com')
                message = 'Có bài đăng gia hạn cần phê duyệt'
                Notification.objects.CreateNotification(adminUser, message)
                return(Response(f'ok'))
...
class Search(APIView):
    class SearchSerializer(serializers.ModelSerializer):
        class Meta:
            model = Post
            fields = ['detailAddress', 'id',
                      'images', 'total_like', 'total_views']

    def get(self, request, searching, begin, end, format=None):
        result = Post.objects.filter(
            detailAddress__contains=searching, is_confirmed=True)[begin:end]
...
class HomePageView(APIView):
    ...
    def get(self, request, location, begin, end, format=None):
        result = Post.objects.filter(
            detailAddress__contains=location, is_confirmed=True)[begin:end]
...
class SearchFilter(django_filters.FilterSet):
    detailAddress = django_filters.CharFilter(
        field_name='detailAddress', lookup_expr='icontains')
    describeAddress = django_filters.CharFilter(
        field_name='describeAddress', lookup_expr='icontains')
    price = django_filters.NumberFilter(field_name='price', lookup_expr='lte')
    square = django_filters.NumberFilter(
        field_name='square', lookup_expr='gte')

    class Meta:
        model = Post
        fields = ['detailAddress', 'describeAddress', 'price', 'roomType',
                  'square', 'kitchen', 'bathroomType', 'heater', 'airconditioner']


class SearchByCiteria(generics.ListAPIView):
    queryset = Post.objects.filter(is_confirmed=True)
...
class ConfirmedPost(APIView):
    ...
    def put(self, request, pk, format=None):
        ...
        if(serializer.is_valid()):
            ...
            message = 'Có bài đăng đã được phê duyệt'
            Notification.objects.CreateNotification(post.hostName, message)
            return Response(f'ok')
        return Response(f'not ok')
